...
if __name__ == "__main__":
    print("모델 선택:")
    print("1: WAGO750-1505")
    print("2: BK20S-T2")
    print("3: BS32c-6A")

    while True:
        model_num = input("모델 번호 (1, 2, 3) : ").strip()

        if model_num not in model_map:
            print("잘못된 모델 번호입니다. 다시 시도하세요.")
            break
        else:
            ...
            try:
                ...
                print("\n=== result ===")
                print(f"Connect count       : {result['status']['connect']}")
                print(f"Disconnect count    : {result['status']['disconnect']}")
                print(f"F1 score            : {result['f1_score']}")
                print(f"Image saved as      : {result['image_name']}")
                print(f"CSV log path        : {result['errata_csv']}")
                print("\n")
